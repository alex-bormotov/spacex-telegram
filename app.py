...
#                 youtube_urls = [i.replace('href="', 'https://youtube.com')
#                                 for i in ress if 'href="/watch?' in i]
#                 print(youtube_urls)
#                 y_urls = [u[:-1]
#                           for u in list(dict.fromkeys(youtube_urls))][:-1]
#                 # print(y_urls)
#                 if len(y_urls) != 0:
#                     return y_urls[0]
...
class Twitter:
    ...
    def get_tweets(self):
        while True:
            sleep(1)
            new_tweets = [
                f"https://twitter.com{tweet['tweetUrl']}" for tweet in get_tweets(self.twitter_username, pages=1)]
            if len(new_tweets) != 0:
                return new_tweets[0]
...
class Reddit:
    def __init__(self):
        ...
        self.reddit = praw.Reddit(client_id=self.reddit_client_id,
                                  client_secret=self.reddit_client_secret, user_agent=self.reddit_user_agent)

    def get_reddit_url(self):
        while True:
            sleep(1)
            reddit_url = [post.url for post in self.reddit.subreddit(
                self.subreddit).new(limit=1)]
            if len(reddit_url) != 0:
                return reddit_url[0]
